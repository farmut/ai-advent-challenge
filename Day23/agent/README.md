# Day 23: RAG-агент с reranker (двухэтапное извлечение)

Расширение Day22. Главное нововведение Day23 — **двухэтапный RAG-пайплайн**: после поиска в базе знаний добавлен этап **reranker** (переоценка релевантности отдельной моделью), а также порог отсечения и раздельный top-K до и после фильтрации.

```
вопрос → поиск (top-K) → rerank → фильтр по порогу → top-K финальных → объединение → LLM
```

Агент по-прежнему умеет всё из предыдущих дней (MCP-инструменты, 3-слойная память, профиль, инварианты, интерактивный FSM). Day22 добавил флаг `--rag`; Day23 добавляет к нему отдельный этап реранкинга (`--rag-rerank`), выделенную модель-реранкер и управление воронкой извлечения.

## Что нового (Day2 → Day23)
...
В Day23 CLI-запрос «заземляется» (grounded) на базе знаний через **двухэтапный** пайплайн:

1. **вопрос** — текст из `--query`;
2. **поиск (retrieve)** — вопрос эмбеддится и по косинусной близости ищется пул из top-K чанков (`--rag-top-k`, теперь это размер пула **до** фильтрации) в SQLite-индексе (`rag.db`), построенном компонентом `rag/`;
...
5. **объединение** — финальные фрагменты + вопрос собираются в единый grounded-промпт (`=== CONTEXT === … === QUESTION === …`) с инструкцией отвечать только по контексту и ссылаться на источники `[n]`;
6. **LLM** — промпт уходит в обычный `ChatUseCase.Execute`.

### Архитектура (Clean Architecture)

| Слой | Компонент |
|---|---|
| domain | `RetrievedChunk` — фрагмент + `Similarity` + `RerankScore`; метод `Score(reranked)` |
| port | `Retriever` — шаг поиска; `Reranker` — шаг переоценки |
| adapter | `internal/adapter/rag/` — HTTP-эмбеддер + read-only поиск по SQLite (`modernc.org/sqlite`); два реранкера: `LLMReranker` (chat-модель как cross-encoder) и `APIReranker` (выделенный `/rerank`-эндпоинт: Cohere/Jina-стиль) |
| usecase | `RAGUseCase` + `RAGConfig` (`TopKRetrieve`/`Rerank`/`Threshold`/`TopKFinal`) → `RAGResult` c разбивкой по этапам; `BuildRAGPrompt` |
...
# Обычный RAG (без reranker)
make run-rag RAG_QUESTION="Какие бывают этапы выполнения очистки таблиц?"

# RAG с reranker (выделенная модель cohere/rerank-4-fast), порогом и воронкой
make run-rag RAG_RERANK=1 RAG_RERANK_MODEL=cohere/rerank-4-fast \
    RAG_TOP_K=10 RAG_THRESHOLD=0.5 RAG_TOP_K_FINAL=3 \
    RAG_QUESTION="Какие бывают этапы выполнения очистки таблиц?"

# или напрямую:
./ai-adv-agent-agent \
    --query "Какие бывают этапы выполнения очистки таблиц?" \
    --rag --rag-db ../rag/rag.db \
    --rag-top-k 10 --rag-rerank --rag-rerank-model cohere/rerank-4-fast \
    --rag-threshold 0.5 --rag-top-k-final 3 \
...
Тесты `t.Skip`, если индекс или эндпоинт эмбеддингов недоступны; `TestRAGCompare` и `TestRAGRerankCompare` требуют `RUN_LLM=1` и LLM-креды (реранк-модель задаётся `RAG_RERANK_MODEL`, по умолчанию — основная LLM).
...
# RAG (Day22–23)
make run-rag                 # Ответ на RAG_QUESTION, заземлённый на RAG_DB (RAG_RERANK=1 — с реранком)
