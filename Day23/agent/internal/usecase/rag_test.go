...
func TestRAGUseCase_BuildPrompt_RerankErrorPropagates(t *testing.T) {
	...
	if _, err := uc.BuildPrompt(context.Background(), "q", RAGConfig{TopKRetrieve: 5, Rerank: true}); err == nil {
		t.Fatal("expected rerank error to propagate")
	}
}
