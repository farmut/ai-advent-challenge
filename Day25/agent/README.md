# Day 24: RAG-агент со структурированным обоснованным ответом (источники + цитаты)

Расширение Day23. Главное нововведение Day24 — на выходе RAG-пайплайна теперь **структурированный обоснованный ответ**: модель возвращает не просто текст, а **ответ + список источников** (файл + раздел/`chunk_id`) **+ цитаты** (дословные фрагменты найденных чанков). Если релевантность найденного ниже порога — ассистент честно говорит **«не знаю»** и просит уточнить вопрос (без вызова LLM). Добавлен **интерактивный RAG-сеанс** (`--rag --interactive`).

```
вопрос → поиск (top-K) → rerank → фильтр по порогу → top-K финальных
       → grounded LLM-вызов → {ответ, источники, цитаты}
       └─ (ничего не прошло порог) → «не знаю, уточните вопрос»
```

Агент по-прежнему умеет всё из предыдущих дней (MCP-инструменты, 3-слойная память, профиль, инварианты, интерактивный FSM). Day22 добавил флаг `--rag`; Day23 — этап реранкинга; Day24 превращает выход в структурированный ответ с цитированием и добавляет интерактивную RAG-сессию.

## Что нового (Day2 → Day24)
...
В Day24 CLI-запрос «заземляется» (grounded) на базе знаний, а на выходе получается **структурированный ответ с цитированием**:
...
| Слой | Компонент |
|---|---|
| domain | `RetrievedChunk` (+ `Similarity` / `RerankScore`, `Score(reranked)`); **`AnswerSource`** (`Source`/`Section`/`ChunkID`/`Score`), **`AnswerQuote`** (+ `Text`), **`RAGAnswer`** (`Answer`/`Sources`/`Quotes`/`Grounded`) |
...
| usecase | `RAGUseCase` + `RAGConfig` (`TopKRetrieve`/`Rerank`/`Threshold`/`TopKFinal`); **`Answer()`** — полный конвейер до структурированного ответа; **`BuildAnswerPrompt`** (JSON-контракт), `parseRAGAnswer`/`salvageRAGAnswerJSON`; `BuildRAGPrompt`/`BuildPrompt` (простой grounded-промпт) сохранены |
...
# Интерактивный RAG-сеанс: вводишь вопросы — получаешь обоснованные ответы
# (по умолчанию реранкинг cohere/rerank-4-fast, 20 → порог 0.5 → 10). /exit — выход.
...
Интерактивный сеанс (`--rag --interactive`): каждый вопрос отвечается тем же grounded-конвейером; `/exit` или `/quit` завершают, `Ctrl-D` (EOF) — тоже.
...
--interactive     Интерактивный режим. Без --rag — FSM-агент (машина состояний);
                  с --rag — RAG-REPL: вопрос → обоснованный ответ (источники + цитаты)
...
make run-rag-interactive     # Интерактивный RAG-сеанс: вопрос → ответ + источники + цитаты (Day24)
