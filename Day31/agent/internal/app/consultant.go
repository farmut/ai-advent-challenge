...
func (c *Consultant) Ask(ctx context.Context, question string) (string, error) {
	...
	tools := c.tb.MCPToolsFor(c.tb.Cfg.Consultant.MCP)
	executor := c.tb.ToolExecutor(c.tb.Cfg.Consultant.MCP)
...
func (c *Consultant) Intro() string {
	...
	if tools := c.tb.MCPToolsFor(c.tb.Cfg.Consultant.MCP); len(tools) > 0 {
