...
type AgentUseCase struct {
	...
	caller     port.MCPToolCaller // optional; enables real tool execution
	in         io.Reader
	out        io.Writer
	err        io.Writer
}
	...
	for ts.Phase != domain.PhaseDone {
		...
		case domain.PhaseExecution:
			...
			if ts.Result == "" {
				...
				// Collect MCP tools if a caller is wired; fall back to plain Execute otherwise.
				var res ChatResult
				var callErr error
				...
				if len(mcpTools) > 0 {
					...
					executor := func(toolCtx context.Context, name, argsJSON string) (string, error) {
						...
						return a.caller.CallTool(toolCtx, srvCfg, name, args)
					}
...
// withMCPToolsContext appends a formatted tool list to the execution system prompt
// so the LLM knows which tools are available and is instructed to use them.
func withMCPToolsContext(prompt string, tools []domain.MCPTool) string {
	...
	sb.WriteString("\n\n## Available Tools\n\n")
	sb.WriteString("You have access to the following tools. To use a tool you MUST invoke it via the function-call mechanism — ")
	sb.WriteString("do NOT describe what you would do or write pseudo-code. ")
	sb.WriteString("Call the function directly and wait for the result before continuing.\n\n")
	for _, t := range tools {
		sb.WriteString(fmt.Sprintf("- **%s**: %s\n", t.Name, t.Description))
	}
	sb.WriteString("\nFor every step that requires external data: call the appropriate tool, observe the result, then proceed.")
	return sb.String()
}
