...
// (verbatim fragments from the retrieved chunks) it relies on.
//
...
func (uc *RAGUseCase) Answer(ctx context.Context, llm port.LLMClient, model string, maxTokens int, question string, cfg RAGConfig) (domain.RAGAnswer, RAGResult, error) {
	...
	if cfg.Rerank && uc.reranker != nil && len(chunks) > 0 {
		...
		if err != nil {
			return domain.RAGAnswer{}, RAGResult{}, fmt.Errorf("rerank: %w", err)
		}
		chunks = rr
		reranked = true
	}
	...
	resp, err := llm.Chat(ctx, port.LLMRequest{
		Model:     model,
		Messages:  []domain.Message{{Role: domain.RoleUser, Content: res.Prompt}},
		MaxTokens: maxTokens,
	})
	if err != nil {
