...
// filter → answer with sources + quotes). /exit or /quit (or EOF) ends the loop.
func runRAGInteractive(ragUC *usecase.RAGUseCase, llmClient port.LLMClient, model string, maxTokens int, ragDB string, cfg usecase.RAGConfig) {
	fmt.Fprintln(os.Stderr, "[rag] Interactive session. Type a question and press Enter.")
	fmt.Fprintln(os.Stderr, "[rag] Commands: /exit or /quit to leave.")
	...
	for {
		...
		ans, res, err := ragUC.Answer(ctx, llmClient, model, maxTokens, question, cfg)
		cancel()
		if err != nil {
...
func main() {
	...
	if *ragEnabled {
		...
		// with the grounded pipeline (answer + sources + quotes).
		if ragInteractive {
			runRAGInteractive(ragUC, llmClient, model, *maxTokens, *ragDB, ragCfg)
			return
		}
