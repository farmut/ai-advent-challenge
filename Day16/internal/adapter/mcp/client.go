...
func listToolsStdio(ctx context.Context, cfg domain.MCPServerConfig) ([]domain.MCPTool, error) {
	...
	cmd.Stderr = os.Stderr // forward server logs to our stderr

	if err := cmd.Start(); err != nil {
		return nil, fmt.Errorf("starting MCP server %q (%s): %w", cfg.Name, cfg.Command, err)
	}
	defer func() {
		stdinPipe.Close()
	...
	// Route incoming JSON-RPC responses by ID.
	...
	go func() {
		...
		for scanner.Scan() {
			...
			if err := json.Unmarshal([]byte(line), &r); err != nil {
				continue // skip unparseable lines (e.g. server startup logs)
			}
			if r.ID == nil {
				continue // notification — no reply needed
			}
			// JSON numbers decode as float64 in interface{}
			var num int64
			switch v := r.ID.(type) {
			case float64:
				num = int64(v)
