...
type tuiState struct {
	...
	// support is non-nil while the session is in customer-support chat mode
	// (/support enters, /end leaves): input lines are answered strictly from the
	// knowledge base, with a link to the source page.
	support *app.Support
...
func (s *tuiState) inputTitle() string {
	...
	if s.support != nil {
		return "Чат поддержки — ответ по базе знаний (Ctrl+S — отправить, /end — выход из режима)"
	}
		...
		case e := <-uiEvents:
			...
			case "<C-s>":
				...
				case cmdHelp:
					...
					if st.support != nil {
						st.appendLog("Сейчас активен режим поддержки. Сначала выйдите из него — /end.")
						break
					}
				...
				case cmdSupport:
					if st.support != nil {
						st.appendLog("Вы уже в режиме поддержки. Выход — /end.")
						break
					}
					if st.consultant != nil {
						st.appendLog("Сейчас активен режим консультанта. Сначала выйдите из него — /end.")
						break
					}
					sp, err := tb.NewSupport()
					if err != nil {
						st.appendLog("⚠ Режим поддержки недоступен: " + err.Error())
						break
					}
					sp.SetOutput(tuiWriter{ch: progressCh})
					st.support = sp
					st.input.Title = st.inputTitle()
					st.appendLog(sp.Intro())
				case cmdEnd:
					switch {
					case st.consultant != nil:
						st.consultant.Close()
						st.consultant = nil
						st.input.Title = st.inputTitle()
					...
					case st.support != nil:
						...
						st.support = nil
						st.input.Title = st.inputTitle()
						st.appendLog("Режим поддержки завершён — снова оркестратор задач.")
					default:
						st.appendLog("Вы не в спец-режиме. /help — консультант, /support — поддержка.")
					}
				case cmdAgents:
				...
				case cmdPerms:
					st.appendLog(orch.PermsSummary())
				case cmdCopy:
					st.copyLastAnswer()
				case cmdSelect:
					st.toggleSelect()
				...
				default: // cmdInput → run the active mode (consultant/support) or the orchestrator
					busy = true
					st.appendLog("… обрабатываю …")
					// Snapshot both modes: they cannot change while busy, and the
					// goroutine must not read the fields the event loop owns.
