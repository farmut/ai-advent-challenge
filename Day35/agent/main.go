...
func printRAGAnswer(ans domain.RAGAnswer) {
	...
	if len(ans.Sources) > 0 {
		...
		for i, s := range ans.Sources {
			...
			// Only when the index carries a link: old-schema indexes (docs.db,
			// shop.db) have no url column, and printing an empty line would be noise.
			if s.URL != "" {
				fmt.Printf("      %s\n", s.URL)
			}
		}
