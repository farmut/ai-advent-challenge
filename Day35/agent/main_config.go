...
func runOrchestrator(configPath string, f *cliFlags) error {
	...
	// Roots and MCP server arguments are resolved against the config file's
	// directory, so the agent behaves the same whatever directory it is
	// launched from. Must run before Validate, which inspects the write roots.
	if err := config.ResolvePaths(&cfg, configPath); err != nil {
		return err
	}
...
func runOrchestratorREPL(orch *app.Orchestrator, tb *app.Toolbelt) error {
	...
	var support *app.Support       // non-nil while in customer-support chat mode
	defer func() {
		if consultant != nil {
			consultant.Close()
		}
	...
	for {
		...
		case cmdHelp:
			...
			if support != nil {
				fmt.Println("\nСейчас активен режим поддержки. Сначала выйдите из него — /end.")
				continue
			}
		...
		case cmdSupport:
			if support != nil {
				fmt.Println("Вы уже в режиме поддержки. Выход — /end.")
				continue
			}
			if consultant != nil {
				fmt.Println("Сейчас активен режим консультанта. Сначала выйдите из него — /end.")
				continue
			}
			sp, err := tb.NewSupport()
			if err != nil {
				fmt.Fprintf(os.Stderr, "Режим поддержки недоступен: %v\n", err)
				continue
			}
			sp.SetOutput(os.Stderr)
			support = sp
			fmt.Println("\n" + support.Intro())
			continue
		case cmdEnd:
			...
			case support != nil:
				support.Close()
				support = nil
				fmt.Println("Режим поддержки завершён — снова оркестратор задач.")
			default:
				fmt.Println("Вы не в спец-режиме. /help — консультант, /support — поддержка.")
			}
			continue
		case cmdAgents:
		...
		// cmdInput — forward to whichever mode is active, else the orchestrator.
		ctx, cancel := context.WithTimeout(context.Background(), 20*time.Minute)
		var answer string
		var err error
		switch {
		...
		case support != nil:
			answer, err = support.Ask(ctx, line)
		default:
