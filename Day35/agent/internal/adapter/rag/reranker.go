...
func (r *LLMReranker) Rerank(ctx context.Context, query string, chunks []domain.RetrievedChunk) ([]domain.RetrievedChunk, error) {
	...
	for i, c := range chunks {
		// Title (when the index has one) helps the scorer place the passage; the
		// chunk's URL is deliberately NOT shown — a link is noise for relevance
		// scoring and only dilutes the passage text the model reasons over.
		label := c.File
		if c.Title != "" && c.Title != c.File {
			label = c.Title
		}
		if c.Section != "" {
			label += " — " + c.Section
