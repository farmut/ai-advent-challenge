...
func Validate(cfg Config) error {
	...
	if err := validateToolPolicy(cfg.Support.Tools, "support"); err != nil {
		return err
	}
	// A malformed max_age must fail loudly at load: silently ignoring it would
	// disable the index-freshness guard exactly when someone meant to enable it.
	if s := strings.TrimSpace(cfg.Support.MaxAge); s != "" {
		if _, err := time.ParseDuration(s); err != nil {
			return fmt.Errorf("invalid support.max_age %q (want a Go duration like \"168h\"): %w", s, err)
		}
	}
	return nil
}
