...
// SupportConfig configures the customer-support chat mode, entered with /support
// and left with /end. It answers strictly from a knowledge-base RAG index (e.g.
// the wiki index) and always cites the source page.
//
// MaxAge / Strict express a security policy, not a convenience: the index is a
// snapshot of the wiki's access rights at index time. Rights change, and a stale
// snapshot can hand a customer a page that has since been restricted. MaxAge is
// a Go duration ("168h"); an empty value disables the check. Strict turns the
// staleness warning into a refusal to start the mode at all — the safe choice
// for a bot facing real customers.
type SupportConfig struct {
	Enabled bool       `yaml:"enabled"`
	RAG     RAGConfig  `yaml:"rag"`     // knowledge-base index; its Enabled gates retrieval
	MCP     []string   `yaml:"mcp"`     // MCP server allow-list; empty = no tools (the default)
	Tools   ToolPolicy `yaml:"tools"`   // per-tool gating; zero value = read-only
	Prompt  string     `yaml:"prompt"`  // optional system-prompt override
	MaxAge  string     `yaml:"max_age"` // e.g. "168h"; empty = no freshness check
	Strict  bool       `yaml:"strict"`  // stale index → refuse to start instead of warning
}
...
func Default() Config {
	return Config{
		...
		// Support is opt-in: it needs a knowledge-base index that most setups do
		// not have. MCP is empty on purpose — a support bot has no business
		...
		Support: SupportConfig{
			...
			MCP:    nil,
			MaxAge: "168h", // a week-old snapshot of wiki ACLs is already suspect
			Strict: false,
		},
	}
}
