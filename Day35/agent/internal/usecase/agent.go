...
func (a *AgentUseCase) Run(cfg ChatConfig) error {
	...
	for {
		...
		switch {
		case strings.ToLower(cmd) == "/help" || strings.ToLower(cmd) == "/помощь" || cmd == "/?":
			fmt.Fprintln(a.out, "Команды: /help /помощь — справка; /resume — продолжить сохранённую задачу;")
			fmt.Fprintln(a.out, "         /restart — сбросить задачу; /pause — пауза; /mcp-* — команды MCP; /exit — выход.")
			fmt.Fprintln(a.out, "Любой другой текст начинает новую задачу.")
