...
func BuildAnswerPrompt(question string, chunks []domain.RetrievedChunk) string {
	...
	b.WriteString("6. Если у фрагмента указан URL — приводи его РОВНО как есть, не сокращай, не дополняй и не выдумывай ссылки.\n\n")
	...
	for i, c := range chunks {
		...
		// URL on its own line, and only when present: an old-schema index has no
		// url column, and an empty "URL:" line would invite the model to invent one.
		if c.URL != "" {
			fmt.Fprintf(&b, "URL: %s\n", c.URL)
		}
...
func parseRAGAnswer(raw string, chunks []domain.RetrievedChunk, reranked bool) domain.RAGAnswer {
	...
	for _, q := range parsed.Quotes {
		...
		// The URL is taken from the chunk, never from the model's reply: a link
		// the model typed could be hallucinated, this one provably came from the index.
