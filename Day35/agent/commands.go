...
const (
	...
	cmdSupport                // /support, /поддержка — enter customer-support chat mode
	cmdEnd                    // /end, /конец — leave the consultant or support mode
	cmdAgents                 // /agents, /агенты
	cmdMemory                 // /memory, /память
	cmdMCP                    // /mcp, /mcp-list
	cmdTools                  // /tools, /инструменты
	cmdPerms                  // /perms, /права — file permission model actually in force
...
var orchestratorCommands = []replCommand{
	...
	{cmdSupport, []string{"/support", "/поддержка"}, "режим чата поддержки: ответ по базе знаний + ссылка на источник"},
	{cmdEnd, []string{"/end", "/конец"}, "выйти из режима консультанта или поддержки"},
	...
	{cmdPerms, []string{"/perms", "/права"}, "показать права на файлы: корни чтения/записи и что можно каждой роли"},
