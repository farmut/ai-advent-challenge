...
func TestRAGGroundedAnswers(t *testing.T) {
	...
	for _, q := range set.Questions {
		...
		// emit the JSON, so a tight cap truncates the structured answer. Retry once:
		// local engines (LM Studio) intermittently 400 on a reasoning model's output
		// ("peg-gemma4 format"), and the reranker is a flaky small model.
		ans, res, err := answerWithRetry(ctx, uc, llmClient, llmModel, q.Question, cfg)
		if err != nil {
...
// answerWithRetry runs the grounded pipeline, retrying once on error. Local
// inference engines intermittently 400 on a reasoning model's output and the
// small local reranker occasionally mangles its scores; a single retry absorbs
// that transient noise without hiding a persistent failure.
func answerWithRetry(ctx context.Context, uc *usecase.RAGUseCase, llm port.LLMClient, model, question string, cfg usecase.RAGConfig) (domain.RAGAnswer, usecase.RAGResult, error) {
	ans, res, err := uc.Answer(ctx, llm, model, 4000, question, cfg)
	if err != nil {
		ans, res, err = uc.Answer(ctx, llm, model, 4000, question, cfg)
	}
	return ans, res, err
