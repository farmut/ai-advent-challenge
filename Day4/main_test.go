...
func captureSendChatRequestWithMessages(provider, model string, maxTokens int, stop []string, messages []Message) (map[string]any, error) {
	...
	_, err := sendChatRequest(provider, "fake-key", srv.URL, model, messages, maxTokens, stop, -1, false)
	if err != nil {
		return nil, err
	}
	if capturedErr != nil {
...
func captureSendChatRequestWithTemperature(provider, model string, maxTokens int, stop []string, temperature float64, query string) (map[string]any, error) {
	...
	_, err := sendChatRequest(provider, "fake-key", srv.URL, model, []Message{{Role: roleUser, Content: query}}, maxTokens, stop, temperature, false)
	if err != nil {
		return nil, err
	}
	if capturedErr != nil {
