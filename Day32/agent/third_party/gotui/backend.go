...
func (b *Backend) Init() error {
	// PATCHED (vendored copy): the -screenshot simulation branch is removed.
	// tcell >= v3.1 dropped NewSimulationScreen, and we vendor gotui exactly to
	// build against tcell v3.4.0 (whose input parser fixes an infinite loop on
	// multi-byte UTF-8 split across read chunks — pasted Cyrillic froze the TUI).
	for i, arg := range os.Args {
		if arg == "-screenshot" {
			b.ScreenshotMode = true
			os.Args = append(os.Args[:i], os.Args[i+1:]...)
			return errors.New("gotui: screenshot/simulation mode is not supported in this build")
		}
	}
...
func (b *Backend) InitWithConfig(cfg *InitConfig) error {
	...
	if cfg.SimulationMode {
		// PATCHED (vendored copy): see Init — simulation screens are gone in
		// tcell >= v3.1.
		return errors.New("gotui: simulation mode is not supported in this build")
	}
