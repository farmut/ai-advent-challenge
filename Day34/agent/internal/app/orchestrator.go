...
func describeAccess(sa *SubAgent) string {
	...
	if len(sa.role.MCP) > 0 {
		...
		// Spell out the write permission: without it the orchestrator happily
		// tells a read-only role to "create the file", and the role can only
		// fail. filepath.Base keeps the prompt short — the roots are absolute.
		if wr := sa.role.Tools.WriteRoot; wr != "" {
			parts = append(parts, "write:"+filepath.Base(wr)+"/")
		} else {
...
func (o *Orchestrator) ToolsSummary() string {
	...
	for _, srv := range servers {
		...
		for _, t := range byServer[srv] {
			// Mark writing tools so the raw pool is not mistaken for what a role
			// may actually call — the guard, not this list, decides that.
			mark := ""
			if specs[t.Name].Kind == policy.KindWrite {
				mark = " [write]"
			}
			if d := strings.TrimSpace(t.Description); d != "" {
				fmt.Fprintf(&b, "  • %s%s — %s\n", t.Name, mark, truncate(d, 80))
			} else {
				fmt.Fprintf(&b, "  • %s%s\n", t.Name, mark)
			}
		}
	}
	b.WriteString("\nЭто полный пул серверов, а НЕ то, что разрешено конкретной роли.\n")
	b.WriteString("Фактические права ролей: /perms")
	return strings.TrimRight(b.String(), "\n")
}

// PermsSummary shows the file permission model actually in force: the roots and,
// per role, which tools that role may call. The /tools list is the raw server
// pool, so without this a reader cannot tell who may write and where.
func (o *Orchestrator) PermsSummary() string {
	tb := o.tb
	cfg := tb.Cfg
	if !cfg.MCP.Enabled || tb.MCPPool == nil {
		return "MCP отключён — файловые инструменты недоступны."
	}

	var b strings.Builder
	b.WriteString("=== Модель прав на файлы ===\n")
	guard := cfg.MCP.FSGuard
	if !guard.Enabled {
		b.WriteString("ВНИМАНИЕ: fs_guard ВЫКЛЮЧЕН — ограничений на пути нет.\n")
	}
	root := guard.ReadRoot
	if root == "" {
		root = "(не задан)"
	}
	fmt.Fprintf(&b, "Чтение/поиск : %s\n", root)
	fmt.Fprintf(&b, "Под guard'ом : %s\n", strings.Join(guard.Servers, ", "))
	if len(guard.Deny) > 0 {
		fmt.Fprintf(&b, "Deny-глобы   : %s\n", strings.Join(guard.Deny, ", "))
	}
	...
	rows := make([]row, 0, len(cfg.Orchestrator.SubAgents)+1)
	for _, sa := range cfg.Orchestrator.SubAgents {
		rows = append(rows, row{sa.Name, Grant{Servers: sa.MCP, Tools: sa.Tools, Role: sa.Name}})
	}
	if cfg.Consultant.Enabled {
		rows = append(rows, row{"consultant (/help)",
			Grant{Servers: cfg.Consultant.MCP, Tools: cfg.Consultant.Tools, Role: "consultant"}})
	}

	specs := policy.DefaultSpecs()
	for _, r := range rows {
		tools := o.tb.MCPToolsForGrant(r.grant)
		var writes []string
		for _, t := range tools {
			if specs[t.Name].Kind == policy.KindWrite {
		...
		where := r.grant.Tools.WriteRoot
		if where == "" {
			where = "запись запрещена"
		}
		fmt.Fprintf(&b, "  • %-12s %d инструмент(ов), запись: %s\n", r.name, len(tools), where)
		if len(writes) > 0 {
			fmt.Fprintf(&b, "      write-тулы: %s\n", strings.Join(writes, ", "))
		}
	}
	b.WriteString("\nПроверка модели прав без LLM: make test-fsguard-live")
	return b.String()
}
