...
// TaskMemoryPath derives the dialogue task-memory file path from the history file path.
func TaskMemoryPath(historyPath string) string { return derived(historyPath, ".taskmem", ".json") }
...
// MCPPath derives the MCP config YAML file path from the history file path.
func MCPPath(historyPath string) string { return derived(historyPath, ".mcp", ".yaml") }
...
// TaskMemoryFile is a file-backed TaskMemoryRepository storing the dialogue task
// memory (goal / clarified points / fixed constraints) as JSON.
type TaskMemoryFile struct{ path string }

// NewTaskMemoryFile creates a TaskMemoryFile at the given path. Empty path is a no-op store.
func NewTaskMemoryFile(path string) *TaskMemoryFile { return &TaskMemoryFile{path} }

func (r *TaskMemoryFile) Load() (domain.TaskMemory, error) {
	if r.path == "" {
		return domain.TaskMemory{}, nil
	}
	var tm domain.TaskMemory
	err := readJSON(r.path, &tm)
	if os.IsNotExist(err) {
		return domain.TaskMemory{}, nil
	}
	if err != nil {
		return domain.TaskMemory{}, fmt.Errorf("invalid task memory file: %w", err)
	}
	return tm, nil
}

func (r *TaskMemoryFile) Save(tm domain.TaskMemory) error {
	if r.path == "" {
		return nil
	}
	tm.UpdatedAt = time.Now().Format(time.RFC3339)
	return writeJSON(r.path, tm)
}
