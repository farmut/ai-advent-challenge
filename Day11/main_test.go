...
// ---------------------------------------------------------------------------
// Day11: working memory load/save
// ---------------------------------------------------------------------------

func TestLoadWM_FileNotExist(t *testing.T) {
	wm, err := loadWM("/tmp/nonexistent_wm_xyz.json")
	if err != nil {
		t.Fatalf("expected nil error for missing file, got: %v", err)
	}
	if wm.Facts == nil {
...
func TestSaveAndLoadWM(t *testing.T) {
	...
	original := workingMemory{Facts: map[string]string{
		"goal":       "Build a REST API",
		"language":   "Go",
		"constraint": "No external libs",
	}}

	if err := saveWM(f.Name(), original); err != nil {
		t.Fatalf("saveWM failed: %v", err)
	}

	loaded, err := loadWM(f.Name())
	if err != nil {
		t.Fatalf("loadWM failed: %v", err)
	}
	...
	if loaded.UpdatedAt == "" {
		t.Error("UpdatedAt should be set after saveWM")
	}
}

func TestSaveWM_EmptyPath(t *testing.T) {
	if err := saveWM("", workingMemory{Facts: map[string]string{"k": "v"}}); err != nil {
		t.Fatalf("saveWM with empty path should be a no-op, got: %v", err)
	}
}
...
// ---------------------------------------------------------------------------
// Day11: wmSystemBlock tests
// ---------------------------------------------------------------------------

func TestWMSystemBlock_Empty(t *testing.T) {
	wm := workingMemory{Facts: map[string]string{}}
...
// ---------------------------------------------------------------------------
// Day11: long-term memory load/save
// ---------------------------------------------------------------------------

func TestLoadLTM_FileNotExist(t *testing.T) {
	ltm, err := loadLTM("/tmp/nonexistent_ltm_xyz.json")
	if err != nil {
		t.Fatalf("expected nil error for missing file, got: %v", err)
	}
	if ltm.Entries == nil {
...
func TestSaveAndLoadLTM(t *testing.T) {
	...
	original := longTermMemory{Entries: map[string]string{
		"user_name":   "Alice",
		"preferred":   "Go over Python",
		"long_goal":   "Build production-grade APIs",
	}}

	if err := saveLTM(f.Name(), original); err != nil {
		t.Fatalf("saveLTM failed: %v", err)
	}

	loaded, err := loadLTM(f.Name())
	if err != nil {
		t.Fatalf("loadLTM failed: %v", err)
	}
	...
	if loaded.UpdatedAt == "" {
		t.Error("UpdatedAt should be set after saveLTM")
	}
}

func TestSaveLTM_EmptyPath(t *testing.T) {
	if err := saveLTM("", longTermMemory{Entries: map[string]string{"k": "v"}}); err != nil {
		t.Fatalf("saveLTM with empty path should be a no-op, got: %v", err)
	}
}
...
// ---------------------------------------------------------------------------
// Day11: ltmSystemBlock tests
// ---------------------------------------------------------------------------

func TestLTMSystemBlock_Empty(t *testing.T) {
	ltm := longTermMemory{Entries: map[string]string{}}
