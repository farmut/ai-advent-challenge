...
func main() {
	...
	ragTopK := flag.Int("rag-top-k", 5, "Number of chunks to retrieve for RAG context")
	...
	// ----------------------------------------------------------------
	// RAG pipeline: question → retrieve relevant chunks → combine with question
	...
	if *ragEnabled {
		...
		ragUC := usecase.NewRAGUseCase(retriever)
		prompt, chunks, err := ragUC.BuildPrompt(ctx, *query, *ragTopK)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Error: RAG retrieval failed: %v\n", err)
			os.Exit(1)
		}
		groundedQuery = prompt

		fmt.Fprintf(os.Stderr, "[rag] retrieved %d chunk(s) from %s:\n", len(chunks), *ragDB)
		...
		if len(chunks) == 0 {
			fmt.Fprintln(os.Stderr, "[rag] no chunks matched — falling back to a plain query")
		}
	}
