...
func ToolDefinitions() []Tool {
	return []Tool{
		...
		{
			Name: "report_prepare_markdown_data",
			Description: "Read the sold-pets JSON report file, aggregate all snapshots into a sales summary " +
				"(unique pets, snapshot trend, min/max counts), and return a fully formatted Markdown document " +
				"as a string. Pass the returned string to report_save_markdown to write it to disk.",
			InputSchema: schema(
				prop{
					"report_file": strProp("Path to the JSON report file created by report_start_collection"),
				},
		...
		{
			Name: "report_save_markdown",
			Description: "Write a Markdown document to a file on disk. " +
				"Typically used after report_prepare_markdown_data: pass its output as 'content' " +
				"and choose a destination path with the .md extension.",
			InputSchema: schema(
				prop{
					"output_file": strProp("Destination path for the Markdown file (e.g. /tmp/sales_report.md)"),
					"content":     strProp("Markdown text to write — usually the output of report_prepare_markdown_data"),
				},
				"output_file", "content",
			),
		},
...
func CallTool(c *Client, name string, args map[string]interface{}) (string, error) {
	...
	case "report_prepare_markdown_data":
		reportFile, err := strArg(args, "report_file")
		if err != nil {
			return "", err
		}
		return PrepareMarkdownData(reportFile)

	case "report_save_markdown":
		outputFile, err := strArg(args, "output_file")
		if err != nil {
			return "", err
		}
		...
		return SaveMarkdownReport(outputFile, content)
