...
// ---- TaskMemoryPath / TaskMemoryFile ----

func TestTaskMemoryPath_Standard(t *testing.T) {
	got := storage.TaskMemoryPath("chat_history.json")
	want := "chat_history.taskmem.json"
	if got != want {
		t.Errorf("TaskMemoryPath(chat_history.json) = %q, want %q", got, want)
	}
}

func TestTaskMemoryPath_Empty(t *testing.T) {
	if p := storage.TaskMemoryPath(""); p != "" {
		t.Errorf("TaskMemoryPath(\"\") should be empty, got %q", p)
	}
}

func TestTaskMemoryFile_SaveLoadRoundTrip(t *testing.T) {
	f, err := os.CreateTemp("", "taskmem_*.json")
	if err != nil {
		t.Fatal(err)
	}
	f.Close()
	defer os.Remove(f.Name())
	repo := storage.NewTaskMemoryFile(f.Name())

	want := domain.TaskMemory{
		Goal:        "разобраться с MVCC",
		Clarified:   []string{"версия 15"},
		Constraints: []string{"только heap"},
	}
	...
	if got.Goal != want.Goal || len(got.Clarified) != 1 || len(got.Constraints) != 1 {
...
func TestTaskMemoryFile_LoadMissingReturnsEmpty(t *testing.T) {
	repo := storage.NewTaskMemoryFile(os.TempDir() + "/taskmem_missing_xyz.json")
	got, err := repo.Load()
	if err != nil {
		t.Fatalf("load: %v", err)
	}
	if !got.IsEmpty() {
		t.Errorf("missing file should load empty memory, got %+v", got)
	}
}
